...
async def test_remove_local_storage_item():
    ...
    result = await handler.remove_local_storage_item(
        tab, "https://example.com", "test_key"
    )
...
async def test_set_local_storage_item_timeout():
    ...
    with pytest.raises(Exception) as exc_info:
        await handler.set_local_storage_item(
            tab, "https://example.com", "key", "value"
        )
...
async def test_remove_session_storage_item():
    ...
    result = await handler.remove_session_storage_item(
        tab, "https://example.com", "session_key"
    )
...
async def test_set_session_storage_item_timeout():
    ...
    with pytest.raises(Exception) as exc_info:
        await handler.set_session_storage_item(
            tab, "https://example.com", "key", "value"
        )
