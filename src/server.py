...
def _hosts_represent_same_site(host_a: str, host_b: str) -> bool:
    ...
    if not host_a or not host_b:
        return False
    return _domain_matches_host(host_a, host_b) or _domain_matches_host(host_b, host_a)


async def _wait_for_target_host(
    tab: Any,
    target_url: str,
    ...
    while True:
        ...
        if _hosts_represent_same_site(target_host, last_host):
            return {
                "matched": True,
...
def _parse_netscape_cookie_line(line: str) -> Optional[Dict[str, Any]]:
    """
...
async def _inject_cookies_from_file(
    tab: Any,
    target_url: str,
    ...
    if not file_path.is_absolute():
        # Try CWD first, then fall back to repo root (src/../) so "cookies.txt"
    ...
    for line in content.splitlines():
        ...
        if host_only:
            ...
            if _domain_matches_host(match_domain, target_host):
                parsed_cookies.append(parsed)
...
async def navigate(
    ...
    Args:
        ...
        inject_cookies (bool): If True, wait for target host and inject cookies from cookies_file.
    ...
    MANDATORY BEHAVIOR when response contains login_required=true:
        ...
        You MUST NOT open a new browser, close this one, try any workaround, or give up.
        The browser is open and visible to the user. They will log in themselves.
    """
    ...
    try:
        ...
        # fallback when pre-injection isn't possible (e.g. file missing).
        pre_injected = False
        if inject_cookies:
            cookie_injection = await _inject_cookies_from_file(tab, url, cookies_file)
        ...
        # Backwards-compatible fallback: if we didn't manage to inject cookies
        # pre-navigation (or injected none), try injecting after reaching the
        # target host and reload.
        if inject_cookies and not pre_injected:
            wait_result = await _wait_for_target_host(
                tab,
                url,
        ...
        # After navigation (with or without cookies), check if we landed on a
        # login page.  If so, register instance as pending manual login and
        # return a special response so the AI agent knows to pause and ask the
