        ...
        try:
            ...
            namespace = {
                ...
                "__builtins__": {
                    ...
                    "sorted": sorted,
                    "enumerate": enumerate,
                    "print": lambda *args: debug_logger.log_info(
                        "hook_function", self.name, " ".join(map(str, args))
                    ),
                },
            }

            # Intentional sandboxed exec with AST validation.
            exec(self.function_code, namespace)  # nosec B102
        ...
        try:
            ...
            if "custom_condition" in self.requirements:
                ...
                try:
                    ...
                    # Restricted eval: no builtins, AST-validated expression only.
            ...
            debug_logger.log_error(
                "dynamic_hook", "matches", f"Error matching hook {self.name}: {e}"
            )
            return False
    ...
    def process(self, request: RequestInfo) -> HookAction:
        ...
        try:
            ...
            debug_logger.log_info(
                "dynamic_hook", "process", f"Processing request {request.url} with hook {self.name}"
            )
            ...
            if isinstance(result, dict):
                ...
                debug_logger.log_error(
                    "dynamic_hook",
                    "process",
                    f"Hook {self.name} returned invalid type: {type(result)}",
                )
                return HookAction(action="continue")

            debug_logger.log_info(
                "dynamic_hook", "process", f"Hook {self.name} returned action: {result.action}"
            )
            return result

        except Exception as e:
            debug_logger.log_error(
                "dynamic_hook", "process", f"Error executing hook {self.name}: {e}"
            )
            return HookAction(action="continue")
...
class DynamicHookSystem:
        ...
        try:
            ...
            for hook in all_hooks:
                ...
                if stage == "response":
                    pattern = uc.cdp.fetch.RequestPattern(
                        ...
                        resource_type=(
                            getattr(uc.cdp.network.ResourceType, resource_type.upper())
                            if resource_type
                            else None
                        ),
            ...
            if not all_patterns:
                all_patterns = [
                    ...
                    uc.cdp.fetch.RequestPattern(
                        url_pattern="*", request_stage=uc.cdp.fetch.RequestStage.RESPONSE
                    ),
                ]
            ...
            debug_logger.log_error(
                "dynamic_hook_system", "setup_interception", f"Failed to setup interception: {e}"
            )
    ...
    async def _on_request_paused(self, tab, event, instance_id: str):
        ...
        try:
            ...
            is_response_stage = (
                hasattr(event, "response_status_code") and event.response_status_code is not None
            ) or (
            ...
            debug_logger.log_info(
                "dynamic_hook_system",
                "_on_request_paused",
                f"Intercepted {stage}: {request.method} {request.url}",
            )
            ...
            if is_response_stage and hasattr(event, "response_status_code"):
                debug_logger.log_info(
                    "dynamic_hook_system",
                    "_on_request_paused",
                    f"Response status: {event.response_status_code}",
                )
            ...
            debug_logger.log_error(
                "dynamic_hook_system",
                "_on_request_paused",
                f"Error processing {stage if 'stage' in locals() else 'request'}: {e}",
            )
            try:
                ...
                debug_logger.log_error(
                    "dynamic_hook_system",
                    "_on_request_paused",
                    f"Failed to continue request: {continue_error}",
                )
                pass
        ...
        try:
            ...
            for hook_id in hook_ids_to_check:
                ...
                if (
                    ...
                    and hook.request_stage == request.stage
                    and hook.matches(request)
                ):
            ...
            if not matching_hooks:
                debug_logger.log_info(
                    ...
                    f"No matching hooks for {request.stage} stage: {request.url}",
                )
            ...
            if request.stage == "response" and event:
                try:
                    body_result = await tab.send(
                        uc.cdp.fetch.get_response_body(
                            request_id=uc.cdp.fetch.RequestId(request.request_id)
                        )
                    )
                    ...
                    debug_logger.log_info(
                        "dynamic_hook_system",
                        "_process_request_hooks",
                        f"Retrieved response body ({len(response_body)} chars)",
                    )
                except Exception as e:
                    debug_logger.log_error(
                        "dynamic_hook_system",
                        "_process_request_hooks",
                        f"Failed to get response body: {e}",
                    )
            ...
            debug_logger.log_info(
                "dynamic_hook_system",
                "_process_request_hooks",
                f"Hook {hook.name} returned action: {action.action}",
            )

            await self._execute_hook_action(
                tab, request, action, event if request.stage == "response" else None
            )

        except Exception as e:
            debug_logger.log_error(
                "dynamic_hook_system", "_process_request_hooks", f"Error processing hooks: {e}"
            )
            try:
                if request.stage == "response":
                    await tab.send(
                ...
                debug_logger.log_error(
                    "dynamic_hook_system",
                    "_process_request_hooks",
                    f"Failed to continue after error: {continue_error}",
                )
                pass
    ...
    async def create_hook(
        ...
        try:
            ...
            debug_logger.log_info(
                "dynamic_hook_system", "create_hook", f"Created hook {name} with ID {hook_id}"
            )
            return hook_id

        except Exception as e:
            debug_logger.log_error(
                "dynamic_hook_system", "create_hook", f"Failed to create hook {name}: {e}"
            )
            raise
        ...
        try:
            async with self._lock:
                if hook_id in self.hooks:
                    ...
                    debug_logger.log_info(
                        "dynamic_hook_system", "remove_hook", f"Removed hook {hook_id}"
                    )
                    return True
            ...
            debug_logger.log_error(
                "dynamic_hook_system", "remove_hook", f"Failed to remove hook {hook_id}: {e}"
            )
            return False
        ...
        try:
            ...
            if action.action == "block":
                ...
                debug_logger.log_info(
                    ...
                    f"Blocked {request.stage} {request.url}",
                )
                ...
                debug_logger.log_info(
                    "dynamic_hook_system",
                    "_execute_hook_action",
                    f"Fulfilled {request.stage} {request.url}",
                )

            elif action.action == "redirect" and request.stage == "request":
                await tab.send(uc.cdp.fetch.continue_request(request_id=request_id, url=action.url))
                debug_logger.log_info(
                    "dynamic_hook_system",
                    "_execute_hook_action",
                    f"Redirected request {request.url} to {action.url}",
                )
                ...
                if request.stage == "response":
                    ...
                    if action.headers:
                        for name, value in action.headers.items():
                            response_headers.append(
                                uc.cdp.fetch.HeaderEntry(name=name, value=value)
                            )
                    ...
                    debug_logger.log_info(
                        "dynamic_hook_system",
                        "_execute_hook_action",
                        f"Modified response for {request.url}",
                    )
                else:
                    headers = []
                    if action.headers:
            ...
            debug_logger.log_error(
                "dynamic_hook_system",
                "_execute_hook_action",
                f"Error executing {request.stage} action: {e}",
            )
            try:
                if request.stage == "response":
                    await tab.send(
                ...
                debug_logger.log_error(
                    "dynamic_hook_system",
                    "_execute_hook_action",
                    f"Failed to continue after action error: {continue_error}",
                )
                pass
