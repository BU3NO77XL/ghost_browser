...
class InMemoryStorage:
    ...
    def _cleanup_orphan_tmp(self):
        ...
        try:
            storage_dir = self._storage_file.parent
            if storage_dir.exists():
                for tmp in storage_dir.glob(".tmp_stealth_storage_*.json"):
                    try:
                        tmp.unlink()
                    except Exception:
                        pass
    ...
    def _load_from_disk(self):
        ...
        with self._lock:
            try:
                ...
                # Safety guard: if file is larger than 1MB something went wrong — reset it
                size = self._storage_file.stat().st_size
                if size > 1 * 1024 * 1024:
                    debug_logger.log_warning(
                        "persistent_storage",
                        "load",
                        f"Storage file is {size // 1024}KB — resetting to prevent startup hang",
                    )
                    self._storage_file.write_text('{"instances": {}}', encoding="utf-8")
                    return
                ...
                # Disk load failed — start with empty in-memory state
