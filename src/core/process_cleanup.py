...
class ProcessCleanup:
    ...
    def _signal_handler(self, signum, frame):
        ...
        debug_logger.log_info(
            "process_cleanup", "signal_handler", f"Received signal {signum}, initiating cleanup..."
        )
    ...
    def _load_tracked_pids(self) -> Dict[str, int]:
        ...
        try:
            ...
            debug_logger.log_warning(
                "process_cleanup", "load_pids", f"Failed to load PID file: {e}"
            )
        return {}
    ...
    def _save_tracked_pids(self):
        ...
        try:
            data = {"browser_processes": self.browser_processes, "timestamp": time.time()}
            with open(self.pid_file, "w") as f:
                json.dump(data, f)
        except Exception as e:
            debug_logger.log_warning(
                "process_cleanup", "save_pids", f"Failed to save PID file: {e}"
            )
    ...
    def _recover_orphaned_processes(self):
        ...
        for instance_id, pid in saved_processes.items():
            if self._kill_process_fast(pid, instance_id):
                killed_count += 1

        if killed_count > 0:
            debug_logger.log_info(
                "process_cleanup", "recovery", f"Killed {killed_count} orphaned browser processes"
            )

        self._clear_pid_file()

    def _kill_process_fast(self, pid: int, instance_id: str = "unknown") -> bool:
        """Kill a process by PID without blocking waits (used at startup)."""
        try:
            ...
            try:
                ...
                if not any(name in proc_name.lower() for name in ["chrome", "chromium", "msedge"]):
                    return False
                proc.kill()  # SIGKILL immediately, no wait
                return True
    ...
    def _kill_process_by_pid(self, pid: int, instance_id: str = "unknown") -> bool:
        ...
        try:
            ...
            try:
                ...
                debug_logger.log_warning(
                    "process_cleanup", "kill_process", f"Could not verify process {pid}: {e}"
                )
            ...
            try:
                ...
                debug_logger.log_error(
                    "process_cleanup", "kill_process", f"Failed to force kill process {pid}: {e}"
                )
                return False
    ...
    def _cleanup_all_tracked(self):
        ...
        if not self.browser_processes:
            debug_logger.log_info(
                "process_cleanup", "cleanup_all", "No browser processes to clean up"
            )
            return
    ...
    def _clear_pid_file(self):
        ...
        try:
            ...
            debug_logger.log_warning(
                "process_cleanup", "clear_pid_file", f"Failed to clear PID file: {e}"
            )
