    ...
    async def extract_complete_element(
        ...
        try:
            debug_logger.log_info(
                "element_cloner",
                "extract_complete",
                f"Starting comprehensive extraction for {selector}",
            )
            ...
            debug_logger.log_info(
                "element_cloner",
                "extract_complete",
                "Executing comprehensive JavaScript extraction",
            )

            result = await tab.evaluate(js_code, return_by_value=True, await_promise=True)

            debug_logger.log_info(
                "element_cloner", "extract_complete", f"Raw result type: {type(result)}"
            )
            ...
            if isinstance(result, dict):
                ...
                debug_logger.log_info(
                    "element_cloner", "extract_complete", "Got RemoteObject, extracting value"
                )
                if hasattr(result, "value") and result.value is not None:
                    extracted_data = result.value
                elif (
                    hasattr(result, "deep_serialized_value")
                    and result.deep_serialized_value is not None
                ):
                    deep_val = result.deep_serialized_value.value
                    debug_logger.log_info(
                        "element_cloner",
                        "extract_complete",
                        f"Deep serialized value type: {type(deep_val)}",
                    )
                    debug_logger.log_info(
                        "element_cloner",
                        "extract_complete",
                        f"Deep serialized value sample: {str(deep_val)[:300]}",
                    )
                    ...
                    if isinstance(deep_val, list) and len(deep_val) > 0:
                        try:
                            ...
                            debug_logger.log_info(
                                "element_cloner",
                                "extract_complete",
                                f"Converted deep serialized to dict with {len(extracted_data)} keys",
                            )
                        except Exception as e:
                            debug_logger.log_error(
                                "element_cloner",
                                "extract_complete",
                                f"Failed to convert deep serialized value: {e}",
                            )
                            extracted_data = {
                                "error": f"Failed to convert deep serialized value: {e}"
                            }
                    else:
                        extracted_data = deep_val
                else:
                    debug_logger.log_error(
                        "element_cloner", "extract_complete", "RemoteObject has no accessible value"
                    )
                    return {
                        "error": "RemoteObject has no accessible value",
                        "remote_object": str(result)[:200],
                    }
            else:
                debug_logger.log_error(
                    "element_cloner", "extract_complete", f"Unexpected result type: {type(result)}"
                )
                return {
            ...
            if not isinstance(extracted_data, dict):
                debug_logger.log_error(
                    "element_cloner",
                    "extract_complete",
                    f"Extracted data is not dict: {type(extracted_data)}",
                )
            ...
            debug_logger.log_error(
                "element_cloner", "extract_complete", f"Error during extraction: {str(e)}"
            )
            return {
