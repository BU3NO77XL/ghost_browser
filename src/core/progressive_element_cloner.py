...
class ProgressiveElementCloner:
    ...
    async def clone_element_progressive(
        ...
        try:
            ...
            debug_logger.log_info(
                "progressive_cloner", "clone_progressive", f"Cloning {selector} -> {element_id}"
            )
    ...
    def expand_events(
        ...
        if event_types:
            events = [
                e for e in events if e.get("type") in event_types or e.get("source") in event_types
            ]
        return {
            "element_id": element_id,
            "data_type": "events",
    ...
    def expand_children(
        ...
        if depth_range:
            ...
            children = [
                c for c in children if isinstance(c, dict) and min_d <= c.get("depth", 0) <= max_d
            ]
